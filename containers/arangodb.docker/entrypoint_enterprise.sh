...
h=$(echo "$ARANGO_LICENSE_KEY" | sha256sum)
if [ "$h" != "eb657fa5838f00359dd937a3f2a501f891d4f4b78bd304d9f7cb2b6ed3ab50f4  -" ] ; then
  echo You need a license key for the ArangoDB enterprise version, which needs
  echo to be stored in the ARANGO_LICENSE_KEY environment variable.
  echo
  echo Please visit:
  echo   https://www.arangodb.com/download-arangodb-enterprise/
  echo to get one for free evaluation.
  echo
  echo Exiting now.
  sleep 10
  exit 1
