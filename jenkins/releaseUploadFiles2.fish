...
function upload
  ...
  and gsutil rsync -x 'index\.html|Packages.*|Release.*' -r $ARANGODB_PACKAGES/repositories/Community/Debian gs://download.arangodb.com/$ARANGODB_REPO/DEBIAN
  and echo "Copying ENTERPRISE"
  and gsutil rsync -x 'index\.html|Packages.*|Release.*' -r $ARANGODB_PACKAGES/repositories/Enterprise/Debian gs://download.arangodb.com/$ENTERPRISE_DOWNLOAD_KEY/$ARANGODB_REPO/DEBIAN
end
