...
function upload
  ...
  and gsutil rsync -c -x 'index\.html|rangodb\.repo|repodata|repomd\..*|.*\.xml\..*|.*\.sqlite\..*' -r $ARANGODB_PACKAGES/repositories/Community/RPM gs://download.arangodb.com/$ARANGODB_REPO/RPM
  and echo "Copying ENTERPRISE"
  and gsutil rsync -c -x 'index\.html|rangodb\.repo|repodata|repomd\..*|.*\.xml\..*|.*\.sqlite\..*' -r $ARANGODB_PACKAGES/repositories/Enterprise/RPM gs://download.arangodb.com/$ENTERPRISE_DOWNLOAD_KEY/$ARANGODB_REPO/RPM
end
