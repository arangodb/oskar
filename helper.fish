...
function findArangoDBVersion
  ...
  # new version scheme (from 3.4.x)  
    ...
    if test "$ARANGODB_VERSION_RELEASE_TYPE" = ""
      ...
      if test "$ARANGODB_VERSION_PATCH" = "devel" \
        ...
        set -xg ARANGODB_VERSION "$ARANGODB_VERSION_MAJOR.$ARANGODB_VERSION_MINOR.0-$ARANGODB_VERSION_PATCH"

        set -xg DOCKER_TAG "$ARANGODB_VERSION_MAJOR.$ARANGODB_VERSION_MINOR-devel"

        set -xg ARANGODB_DARWIN_UPSTREAM "$ARANGODB_VERSION_MAJOR.$ARANGODB_VERSION_MINOR.0"
        set -xg ARANGODB_DARWIN_REVISION "$ARANGODB_VERSION_PATCH"
        ...
        set -xg ARANGODB_TGZ_UPSTREAM "$ARANGODB_VERSION_MAJOR.$ARANGODB_VERSION_MINOR.0-$ARANGODB_VERSION_PATCH"
  ...
  echo
  echo "DARWIN_UPDATE/REVISION:   $ARANGODB_DARWIN_UPSTREAM / $ARANGODB_DARWIN_REVISION"
  echo "DEBIAN_UPSTREAM/REVISION: $ARANGODB_DEBIAN_UPSTREAM / $ARANGODB_DEBIAN_REVISION"
  echo "DOCKER_TAG:               $DOCKER_TAG"
  echo "PACKAGES:                 $ARANGODB_PACKAGES"
  echo "REPOSITORY:               $ARANGODB_REPO"
  echo "RPM_UPDATREAM/REVISION:   $ARANGODB_RPM_UPSTREAM / $ARANGODB_RPM_REVISION"
