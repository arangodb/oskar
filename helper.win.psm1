#
# Copyright (C) ArangoDB GmbH, 2025, Cologne, Germany
#
# This script is intended to be used to build ArangoDB-3.12.x inside the ArangoDB windows build container as container has all the tools and CLang and MSVC preinstalled.
#
$global:WORKDIR = $pwd
...
{
    Write-Host "setVisualStudioEnvs called with $args"
    # param (
...
Function VS2019
{
    Write-Host "VS2019 called with $args"
}

Function VS2022
{
    Write-Host "VS2022 called with $args"
...
#     # VS2019
#     Write-Host "findCompilerVersion called with $args"
# }
...
{
    Write-Host "proc called with $process  $argument $logfile $priority"
    If (!$priority)
    {
        $priority = "Normal"
    }
...
Function configureWindows
{
    ...
    Else
    {
        ...
        $process_args = "--fresh --preset $cmake_preset -DVERBOSE=On -DUSE_MAINTAINER_MODE=`"$MAINTAINER`" -DUSE_GOOGLE_TESTS=`"$MAINTAINER`" -DUSE_CATCH_TESTS=`"$MAINTAINER`" -DUSE_ENTERPRISE=Off -DCMAKE_BUILD_TYPE=`"$BUILDMODE`" -DPACKAGING=NSIS -DCMAKE_INSTALL_PREFIX=/ -DSKIP_PACKAGING=`"$SKIPPACKAGING`" -DUSE_FAILURE_TESTS=`"$USEFAILURETESTS`" -DSTATIC_EXECUTABLES=`"$STATICEXECUTABLES`" -DOPENSSL_USE_STATIC_LIBS=`"$STATICLIBS`" -DUSE_STRICT_OPENSSL_VERSION=On -DBUILD_REPO_INFO=`"$BUILD_REPO_INFO`" -DTHIRDPARTY_BIN=`"$ARANGODIR_SLASH/build/arangodb.exe`" -DUSE_CLCACHE_MODE=`"$CLCACHE`" -DUSE_CCACHE=`"Off`" -DARANGODB_BUILD_DATE=`"$ARANGODB_BUILD_DATE`" $global:CMAKEPARAMS `"$global:ARANGODIR`""
...
Function signWindows
{
    ...
    {
        Write-Host "Sign: signtool.exe sign /sm /fd sha256 /td sha256 /sha1 D4F9266E06107CF3C29AA7E5635AD5F76018F6A3 /tr `"http://timestamp.digicert.com`" `"$PACKAGE`""
        proc -process signtool.exe -argument "sign /sm /fd sha256 /td sha256 /sha1 D4F9266E06107CF3C29AA7E5635AD5F76018F6A3 /tr `"http://timestamp.digicert.com`" `"$PACKAGE`"" -logfile "$INNERWORKDIR\$($PACKAGE.Split('\')[-1])-sign.log" -priority "Normal"
    }
    Pop-Location
}
