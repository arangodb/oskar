...
if test "$STARTER_REV" = latest
  set -l meta (curl -s -L "https://api.$ARANGODB_GIT_HOST/repos/$STARTER_GIT_ORGA/arangodb/releases/latest")
...
echo "https://$ARANGODB_GIT_HOST/$STARTER_GIT_ORGA/arangodb/releases/download/$STARTER_REV/arangodb-$PLATFORM-$arch"
and curl -s -L -o "$STARTER_PATH" "https://$ARANGODB_GIT_HOST/$STARTER_GIT_ORGA/arangodb/releases/download/$STARTER_REV/arangodb-$PLATFORM-$arch"
