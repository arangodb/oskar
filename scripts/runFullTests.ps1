...
Function global:registerSingleTests()
{
    ...
    if (Test-Path "$INNERWORKDIR/ArangoDB/tests/test-definitions.txt" -PathType Leaf) {
        ...
        python "$env:WORKSPACE\jenkins\helper\generateJenkinsScripts.py $INNERWORKDIR\ArangoDB\tests\test-definitions.txt -f ps1 --full | Invoke-Expression
    } else {
...
Function global:registerClusterTests()
{
    ...
    if (Test-Path "$INNERWORKDIR/ArangoDB/tests/test-definitions.txt" -PathType Leaf) {
        Write-Host "Using test definitions from repo..."
        python "$env:WORKSPACE\jenkins\helper\generateJenkinsScripts.py $INNERWORKDIR\ArangoDB\tests\test-definitions.txt -f ps1 --full --cluster | Invoke-Expression
    } else {
