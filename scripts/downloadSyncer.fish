...
if test "$SYNCER_REV" = latest
  set -l meta (curl -s -L -H "Authorization: $DOWNLOAD_SYNC_USER" "https://api.github.com/repos/arangodb/arangosync/releases/latest")
...
set -l meta (curl -s -L -H "Authorization: $DOWNLOAD_SYNC_USER" https://api.github.com/repos/arangodb/arangosync/releases/tags/$SYNCER_REV)
...
curl -s -L -H "Accept: application/octet-stream" -H "Authorization: $DOWNLOAD_SYNC_USER" "https://api.github.com/repos/arangodb/arangosync/releases/assets/$asset_id" -o "$SYNCER_PATH"
and chmod 755 "$SYNCER_PATH"
and echo Syncer ready for build $SYNCER_PATH
or begin echo "ERROR - cannot download Syncer"; exit 1; end
